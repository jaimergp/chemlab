...
class InstanceArray(InstanceProperty):
    ...
    def reorder(self, neworder, inplace=True):
        if self.value is None:
            ...
            if len(neworder) != self.size:
                raise ValueError("'%s': neworder doesn't have enough elements %d (value %d)" % (
                    self.name, len(neworder), self.size))

            if set(neworder) != set(range(self.size)):
                raise ValueError(
                    "'%s': the new order is invalid as it doesn't contain all the indices." % self.name)
    ...
    def sub(self, index):
        ...
        if self.size < len(index):
            raise ValueError('Can\'t subset "{}": index ({}) is bigger than the number of elements ({})'.format(
                self.name, len(index), self.size))
...
class InstanceAttribute(InstanceArray):
    ...
    def field(self, index):
        obj = InstanceField(
            name=self.name, dtype=self.dtype, shape=self.shape, alias=self.alias)
    ...
    def append(self, attr_or_field):
        ...
        if isinstance(attr_or_field, InstanceAttribute):
            ...
            if self.value is None:
                ...
                self.value = np.append(
                    self.value, [attr_or_field.value], axis=0)
        else:
            raise ValueError(
                'Can append only InstanceAttribute or InstanceField')
        ...
        if self.shape is None:
            ...
            if value is None:
                ...
                if self.shape != value.shape:
                    raise ValueError('Field {}, shape mismatch: got {} instead of {}'.format(
                        self.name, value.shape, self.shape))
                self._value = value
